...
//                    return wasLast;
                ...
                protected static<E> boolean add(AtomicReference<Snapshot<E>> atomicReference, E element) {
                    ...
                    do {
                        prev = atomicReference.get();
                        next = Snapshot.add(element, prev);
                        first = prev.size == 0;
                    } while (!atomicReference.compareAndSet(prev, next));
                    return first;
                }
                ...
                private static<T> Snapshot<T> add(T e, Snapshot<T> snapshot) {
                    ...
                    int nextSize, prevSize = snapshot.size, nextVersion = snapshot.version + 1;

                    nextArray = prevSize == prevArray.length ?
    ...
    private static final class SnapshottingImpl<E, S> implements SimpleList.LockFree.Snapshotting<E, S> {
        ...
        public Pair.Immutables.Bool<S> add(E element) {
            return SimpleList.LockFree.Snapshot.add(core, element, sSupplier);
        }
