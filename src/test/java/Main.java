...
//        });
        ...
        final FastSubscriptionTest test = new FastSubscriptionTest();

        Consumer<String> owner = x -> System.err.println("MAIN: OBSERVED: " + x);

        Switchers.Switch aSwitch = test.watch(owner);

        aSwitch.on();

        final LockFreeChronometer chronometer = new LockFreeChronometer(TimeUnit.MILLISECONDS);

        chronometer.start();
        postpone(
                2000,
                () -> {
                    System.err.println("MAIN: TURNING OFFF!!!! at: " + chronometer.lapse());
                    aSwitch.off();
                    test.changeState();
                    postpone(
                            100, //
...
//                            102, // ERROR (ONCE)
//                            103, // ERROR (ONCE)
//                            110, // PASS -> (FAILS RETREIVE)
//                            90, // ERROR
//                            80, // PASS -> (FAILS RETREIVE)
//                            90, // ERROR
//                            200, // PASS -> (FAILS RETREIVE)
//                            600, // PASS -> (FAILS RETREIVE)
//                            50, // PASS -> (FAILS RETREIVE!!!)
//                            100, // ERROR
                            () -> {
                        System.err.println("MAIN: ON AGAIN!!! at: " + chronometer.lapse());
                                aSwitch.on();
                                postpone(
                                        1000, () -> {
